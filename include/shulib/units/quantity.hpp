#pragma once
//
// Quantity<L, A, T, E> — compile-time dimensional analysis.
//
// The integer exponents of the four base dimensions live in the TYPE:
...
template <int L, int A, int T, int E>
class Quantity {
public:
...
template <int L1, int A1, int T1, int E1, int L2, int A2, int T2, int E2>
[[nodiscard]] constexpr auto operator*(Quantity<L1, A1, T1, E1> a, Quantity<L2, A2, T2, E2> b) noexcept
    -> Quantity<L1 + L2, A1 + A2, T1 + T2, E1 + E2> {
    return Quantity<L1 + L2, A1 + A2, T1 + T2, E1 + E2>{a.value() * b.value()};
}
...
using Number          = Quantity<0, 0, 0, 0>;   // dimensionless
using Length          = Quantity<1, 0, 0, 0>;   // inches
using AngleDim        = Quantity<0, 1, 0, 0>;   // radians (rates/bookkeeping; headings use math::Angle)
using Time            = Quantity<0, 0, 1, 0>;   // seconds
using Voltage         = Quantity<0, 0, 0, 1>;   // volts
using Velocity        = Quantity<1, 0, -1, 0>;  // in/s
using Acceleration    = Quantity<1, 0, -2, 0>;  // in/s^2
using AngularVelocity = Quantity<0, 1, -1, 0>;  // rad/s
