...
/// The human-readable terminal stream and the primary dev surface: leveled tagged lines from
/// log(), one column-aligned line per DebugRecord from emit(), and the one-screen block from
/// summarize(). It holds NO mutable state — no buffer, no queue, no background flush — so every
/// LINE is formatted into a stack-local buffer and handed to the ICharSink as exactly ONE write():
/// one for log(), one for emit(), six for summarize()'s block. A line can therefore never be torn
/// or interleaved mid-way, whatever a caller's string contains; whole LINES can be, so two tasks
/// sharing a sink can split summarize()'s six. Nothing here allocates. Every line is pinned by a
/// golden test, which is why the character device is injected rather than hard-coded to stdout.
/// This is a DISPLAY edge: degrees are rendered here and only here, and only the headline fields
/// appear — the full record belongs to the blackbox and SHUL/2 sinks.
...
public:
    ...
    /// One leveled, subsystem-tagged line, stamped from the injected CLOCK (unlike emit(), which
    /// stamps from the record). Info carries no level tag — the common case stays quiet — while
    /// the others render [LEVEL] butted against [TAG]. Both `subsystem` and `message` are
    /// caller-controlled text and are SANITIZED on the way out: control bytes become '?', bytes
    /// >= 0x80 pass through, and over-long text truncates with '…' at a UTF-8 boundary (16 and 200
    /// bytes). There is no unsanitized path to the device, so no caller string can break the
    /// one-line-per-call framing or escape into the terminal.
    ...
    /// makes a replayed record render byte-identically to a live one. The tick is tagged [MOT]
    /// when it carries a command id OR a non-idle motion state (a C1 motion running standalone has
    /// id 0, so discriminating on the id alone once rendered an ACTIVE motion as "[LOC] idle"),
    /// and "[LOC] idle" only when both are zero. Trailing flags are appended ONLY when set: " DR",
    /// " SFB", " CLMP", " flt=NAME". Angles print in degrees, and a non-finite value renders as a
    /// deterministic "NaN"/"+Inf"/"-Inf" token rather than libc's locale-varying spelling.
