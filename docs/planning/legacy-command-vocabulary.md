# Legacy command vocabulary — mined at C6, the last look before C7 deletes `legacy/`
...
>
> **Method:** extracted **mechanically** (scripts in §6, re-runnable until C7 deletes the files),
> from all four places commands exist in `legacy/`; the hand-reading only interprets what the
> scripts found. Nothing below is from memory.
...
## 6. Mechanical cross-check (re-runnable until C7)

Every count above came from these commands, run at C6 (2026-08-10), from the repo root:
...
lengths with an awk pass over `NONE` positions. See `chunks/C6-PROGRESS.md` for the live log of
each extraction as it was run.

---

*Produced by chunk C6 (2026-08-10). Companion documents: [`chunks/C6-COMPLETED.md`](chunks/C6-COMPLETED.md)
(the full 34-file audit + safe-to-delete verdict), [`hardware-assumptions.md`](hardware-assumptions.md)
(the legacy-measured reference constants, added at C6), [`shulib-v2-master-plan.md`](shulib-v2-master-plan.md)
§16.3 (the registry design this feeds), [`build-order.md`](build-order.md) G2/G4.*
