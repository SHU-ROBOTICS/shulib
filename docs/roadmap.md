...
> hope).
> *(**Both halves of that last sentence have moved on, and the record is left standing rather
> than rewritten.** The site went live at DOCS1 — docs.shurobotics.com, HTTPS enforced — and
> DOCS2 took the reference from the two frozen surfaces to the whole public API, so M7's
> publish clause is `[x]`. The cookbook-cold-read clause is still open, still owned by the
> programming chair.)*
...
- [x] **Generated API docs** *(chunk D3, 2026-08-12; published at DOCS1, 2026-08-14; expanded to
  the whole public surface at DOCS2, 2026-08-14)*: `docs/api/` is generated from the headers by
  `tools/api_doc_tool.py` — deterministic, committed, and verified up to date by a build-time
  regeneration check; a **doc-coverage gate** fails the build naming any public entity with no
  documentation; a C++ fidelity pin (`test/api_reference_fidelity_test.cpp`) holds the rendered
  signatures to the headers.
  **Published** at docs.shurobotics.com over enforced HTTPS, from `main`, with `check-fresh` run
  before the site renders — publishing a stale reference is worse than publishing none.
  **DOCS2 (2026-08-14) took it from 2 documented types to the whole public API**: the target list
  became a glob over `include/shulib/`, so the reference is 117 pages covering **1,625 public
  entities** — every type, member, nested type, free function, namespace-scope constant and type
  alias — and every one of them is under the coverage gate. Getting there needed the parser
  rebuilt: it had been structurally blind to any type with a base-class list, any enum with an
  underlying type, everything at namespace scope, and public nested types (which it refused
  outright), so **three LOCKED contracts — the coordinate frame, the accuracy targets and the
  units vocabulary — had been invisible to it in their entirety**. The mkdocs nav is generated
  from the same target list and byte-checked, because a page absent from the nav was measured to
  ship unreachable with exit code 0.
