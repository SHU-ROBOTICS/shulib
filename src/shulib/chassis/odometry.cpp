...
shulib::Pose odomLocalSpeed(0, 0, 0);
...
void shulib::update() {
 ...
 /* leftTotalTravel += std::fabs(dL);
  ...
  // Left wheel stuck detection (rotation sensor port 10, offset -6.5")
  if (leftZero && otherWheelsMovingForLeft) {
    leftZeroCount++;
    if (leftZeroCount >= STUCK_THRESHOLD && !leftStuckWarned) {
      shulib::logger().error("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
      shulib::logger().error("[STUCK] LEFT ODOM WHEEL (port 10, left side, offset -6.5\")");
  ...
  // Right wheel stuck detection (rotation sensor port -8, offset 6.5")
  if (rightZero && otherWheelsMovingForRight) {
    rightZeroCount++;
    if (rightZeroCount >= STUCK_THRESHOLD && !rightStuckWarned) {
      shulib::logger().error("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
      shulib::logger().error("[STUCK] RIGHT ODOM WHEEL (port 8, right side, offset 6.5\")");
      shulib::logger().error("[STUCK] Zero readings for " +
  ...
  // Back wheel stuck detection (rotation sensor port 9, offset -4.0")
  if (backZero && otherWheelsMovingForBack) {
    backZeroCount++;
    if (backZeroCount >= STUCK_THRESHOLD && !backStuckWarned) {
      shulib::logger().error("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
      shulib::logger().error("[STUCK] BACK ODOM WHEEL (port 9, rear, offset -4.0\")");
      shulib::logger().error("[STUCK] Zero readings for " +
